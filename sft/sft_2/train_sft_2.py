...
from sft.sft_2.sft_config_2 import SFTModelConfig, SFTDataConfig, SFTTrainerConfig
...
def _setup_local_cuda_device() -> None:
    """
    Fixes NCCL 'Duplicate GPU detected' by binding each rank to its local GPU.
    Must be called BEFORE any NCCL collectives (including broadcast/barrier).
    """
    ...
    try:
        ...
        # If something is off in the environment, don't hard-crash here;
        # but NCCL may fail later anyway.
        return
...
def build_tokenizer(model_cfg: SFTModelConfig) -> Any:
    ...
    if tokenizer_path and os.path.exists(tokenizer_path):
        ...
        fallback_paths = ["/from_s3/tokenizer/", "/from_s3/sft1_model/"]
        resolved_path = None
...
def build_model(model_cfg: SFTModelConfig, tokenizer: Any, ds_config: Dict[str, Any]) -> Any:
    ...
    if model_cfg.checkpoint_path and os.path.exists(model_cfg.checkpoint_path):
        if _is_rank0():
            print(f"Loading SFT Stage 1 weights from {model_cfg.checkpoint_path}")
        ...
        if _is_rank0():
            print(f"Successfully loaded weights. Missing keys: {len(missing)}, unexpected: {len(unexpected)}")
...
def build_datamodule(data_cfg: SFTDataConfig, tokenizer: Any) -> SFTDataModule:
    ...
    status_path = os.path.join(data_cfg.data_root, ".prep_status_sft2.json")
    prep_timeout_s = int(os.environ.get("SFT_DATA_PREP_TIMEOUT_S", "21600"))  # 6h default
    poll_s = float(os.environ.get("SFT_DATA_PREP_POLL_S", "5"))

    if not train_files and _is_rank0():
        print("SFT Stage 2 data not found. Running data preparation mixture...")
        try:
            prep_cfg = SFT2DataPrepConfig(
            ...
            prepare_sft2_data(prep_cfg, tokenizer=tokenizer)
            with open(status_path, "w") as f:
                json.dump({"ok": True, "error": None}, f)
        except Exception as e:
            print(f"CRITICAL: Data preparation failed on rank 0: {e}")
            # Write status for other ranks so they don't hang
            try:
                with open(status_path, "w") as f:
    ...
    # Avoid NCCL collectives while rank0 is downloading/generating huge datasets:
    # poll a shared status file instead.
    if not train_files:
        import time
        start = time.time()
        while True:
            if os.path.exists(status_path):
                ...
                if not status.get("ok", False):
                    raise RuntimeError(f"Data preparation failed on rank 0: {status.get('error')}")
                break
            if time.time() - start > prep_timeout_s:
                raise TimeoutError(
                    f"Timed out waiting for data preparation status file: {status_path} "
                    f"(waited {prep_timeout_s}s)"
                )
            time.sleep(poll_s)
...
def convert_to_hf(output_dir: str, tag: str, config: Any, tokenizer: Any):
    ...
    try:
        ...
        if isinstance(state_dict, dict):
            ...
            elif "module" in state_dict: state_dict = state_dict["module"]
        ...
        for key, tensor in state_dict.items():
            ...
            new_key = key
            if new_key.startswith("module."): new_key = new_key[len("module.") :]
        ...
        hf_config = {
            ...
            "rms_norm_eps": getattr(config, "rms_norm_eps", 1e-5), # Fixed consistency
...
@torch.no_grad()
def run_eval(model_engine: Any, val_loader: Any, device: torch.device, max_batches: int = 50) -> float:
    model_engine.eval()
...
def main() -> None:
    ...
    if data_cfg.max_length > model_cfg.max_position_embeddings:
        raise ValueError(
            f"data_cfg.max_length ({data_cfg.max_length}) exceeds "
            f"model_cfg.max_position_embeddings ({model_cfg.max_position_embeddings})."
        )
    ...
    if torch.distributed.get_rank() == 0:
        task = Task.init(project_name="LLaMA_SFT", task_name="SFT_Stage_2_Task_Solvers", reuse_last_task_id=False)
